...
class CNN:
    def __init__(self, descriptor_width=DESCRIPTOR_WIDTH,
        ...
        self.model.add(Convolution2D(nb_filters*2, nb_conv, nb_conv, border_mode='same',input_shape=input_shape))
        self.model.add(Activation('relu'))
        self.model.add(Convolution2D(nb_filters*2, nb_conv, nb_conv, border_mode='same'))
        self.model.add(Activation('relu'))
        self.model.add(MaxPooling2D(pool_size=(nb_pool, nb_pool), strides=(nb_pool, nb_pool)))
        self.model.add(Dropout(0.25))


        self.model.add(Convolution2D(nb_filters, nb_conv, nb_conv, border_mode='same'))
        self.model.add(Activation('relu'))
        self.model.add(Convolution2D(nb_filters, nb_conv, nb_conv, border_mode='same'))
        self.model.add(Activation('relu'))
        self.model.add(MaxPooling2D(pool_size=(nb_pool, nb_pool), strides=(nb_pool, nb_pool)))
